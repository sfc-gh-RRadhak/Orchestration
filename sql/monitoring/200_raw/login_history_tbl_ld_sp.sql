insert into {environment}_plt_rl_snowflake_db.info_schema.login_history
            with l_stg as
            (
                select 
                    ...
                    ,s.*
                from
                    {environment}_plt_rl_snowflake_db.info_schema.login_history_stg s
            )
            ,l_deduped as
            (
                select
            ...
            where
                ...
                (
                    select dw_event_shk from {environment}_plt_rl_snowflake_db.info_schema.login_history
                )
            order by
                event_timestamp;